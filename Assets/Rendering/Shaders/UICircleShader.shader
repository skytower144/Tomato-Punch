Shader "UI/CircleMask"
{
    Properties
    {
        ...
        _RadiusX ("Radius X", Range(0,2)) = 1
        _RadiusY ("Radius Y", Range(0,2)) = 1
        
        _ScaleX ("Scale X", Float) = 1
        _ScaleY ("Scale Y", Float) = 1

        _CenterX ("Center X", Range(-1,1)) = 0.5
        _CenterY ("Center Y", Range(-1,1)) = 0.5
    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            float circleDelta(float2 texcoord) {
                ...
                float2 centeredTexcoord = texcoord - float2(_CenterX, _CenterY);
                float x = (2 * (centeredTexcoord.x - 0.5)) / _RadiusX;
                float y = (2 * (centeredTexcoord.y - 0.5)) / _RadiusY;
                float value = x * x + y * y;
